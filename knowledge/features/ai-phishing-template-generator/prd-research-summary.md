> **Update 2026-07-08:** Four items below were resolved with the feature owner — see `open-questions.md` for full answers. Summary: Copy an Email uses the same permission tier as Describe a Scenario (no separate gate); no mandatory attestation step before deploying an email-sourced template; image-embedded sensitive data gets no special handling in v1 (redaction is text-based only); and manual edits are always preserved — difficulty changes never touch an existing draft, and getting a different-difficulty variant requires an explicit Regenerate action. The sections below reflect the original critique as written; treat those four points as superseded by the resolutions above.
