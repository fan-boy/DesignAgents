...
- **Resolved and built:** difficulty as a pre-generation input, labeled Generating progress, desktop/mobile preview toggle, redaction/safe-link confirmation banner, Regenerate confirmation modal, Asset Detail with RBAC-gated deploy, Delete confirmation modal, an **Add Phishing Link** three-state sub-flow (select text → configure popover with auto safe tracking URL → tracked-link token + confirmation), and an **Add Variable** (personalization) three-state sub-flow (Insert variable toolbar control → searchable grouped menu → atomic token chip with sample-data preview and per-variable fallback). Add QR code exists in the toolbar as a sibling capability.
- **Personalization variables — foolproof-by-design:** admins never type merge-tag syntax. An Insert variable toolbar control opens a searchable menu of plain-English variables grouped Recipient / Location / Company; one click inserts an atomic token chip (un-typo-able, deletes as one unit), the Preview shows it filled with sample data, and each variable carries a per-variable fallback so missing recipient data never leaks raw tokens into a live simulation.
- **As-built screen inventory (21 frames):** three creation tabs (each with setup + post-generation edit variants), Generating progress, Mobile preview, Regenerate confirm modal, Library empty state, Library view-only state, Library row-actions menu (with Delete), Asset Detail (deploy-enabled), Asset Detail (RBAC-disabled), Delete confirm modal, the 3-state Add Phishing Link sub-flow, and the 3-state Add Variable sub-flow. Delete is reachable from both the library row Actions menu and Asset Detail, sharing one confirmation modal.
