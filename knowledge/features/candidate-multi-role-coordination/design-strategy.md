...
**Update (2026-07-11, supporting screens + Para AI attribution):** added three screens around the core candidate experience — Home, Candidates, Opportunities (list) — purely as supporting context, explicitly not part of what the brief is asking to be evaluated on (see "Supporting screens" below). Also resolved the open question of where a recruiter's cross-candidate review backlog lives: the Home dashboard's "Needs your review" feed, not a new dedicated destination. Added explicit Para AI attribution (a persistent sidebar indicator plus labeled tags on its actual output) so its presence stays legible now that it has no page of its own.
...
- Para AI's matching is assumed to extend beyond opportunities the recruiter has directly submitted the candidate to (a broader, platform-wide matching layer). Any such surfaced match is a recommendation only — it becomes an active Opportunities-view track solely on recruiter confirmation, the same gate as a recruiter-initiated submission.
...
- Matching-inclusion defaults: resume/LinkedIn default Included in matching (low-sensitivity, largely public data already). Notes and calls default excluded, with a one-tap "Include this in matching?" prompt surfaced right when the item is captured — this is the mechanism meant to prevent the feature from starving the model of context just because inclusion was buried in a settings toggle nobody visits.
...
## Supporting screens (context, not the core deliverable)
The brief asks specifically for the recruiter-plus-Para-AI experience of managing one candidate across multiple opportunities — that's the Opportunities and Profile & Context screens above, and they're what should actually be evaluated against the prompt. Three additional screens were built around them purely as supporting context, so the core experience doesn't read as a floating screen with no product around it:

- **Home** — a recruiter dashboard (12-column grid, 8/4 split): quick stat tiles, a "Needs your review" feed aggregating Para-AI-drafted actions across every candidate the recruiter owns (this is what resolves the global-surface question below), an "Opportunities at a glance" panel (stage-distribution chart, most-active roles), and a lighter "Candidates needing attention" list.
- **Candidates** — a searchable list of every candidate the recruiter is handling, each row flagging active-opportunity count and pending-review count.
- **Opportunities** — a list of open roles across the recruiter's book, with pipeline stage breakdown per role.

These three answer "what does a recruiter see before they get to a specific candidate," which the brief doesn't ask for directly. Treat them as scaffolding that makes the core screens feel real, not as part of what's being graded.

**Para AI attribution.** Once Para AI's presence moved off a dedicated page and into inline surfaces, a new risk appeared: its work could become invisible precisely because it's no longer behind a labeled destination. Two things address this without reintroducing a separate page — a persistent "Para AI active" indicator in the shared sidebar (present on every screen, not clickable, just a status signal), and explicit "✨ Para AI" tags plus a "Drafted by Para AI" subtitle wherever its actual output appears (the Home feed, the opportunities-at-a-glance ranking). The goal: a recruiter can always tell what came from Para AI versus what's plain system data, without Para AI needing a page to point at.

## Global surface (resolved)
A recruiter's cross-candidate backlog — every approval-required item across every candidate they own, not just this one — needed a single entry point once the per-candidate Action Queue was removed. This is now answered by the Home screen: its "Needs your review" feed aggregates Para-AI-drafted actions across every candidate, and the "Needs your review" stat tile gives a persistent count. No separate dedicated destination was needed — the dashboard already had the right shape for it.
...
- The Home dashboard's stage-distribution chart and "most active" ranking are illustrative — the actual logic for what counts as "most active" (recency of activity vs. proximity to a deadline vs. recruiter-assigned priority) hasn't been defined.
