...
- Candidate active in a high number of opportunities (8-10+) — parallel-track layout scaling
- A note or call recording is marked private after Para AI already used it in a recommendation
...
- Bulk-approve flow — what the recruiter sees per item before a batch send
- Candidate withdraws from the process entirely — all tracks close, all queued actions across tracks cancelled
- Para AI surfaces a new opportunity match the recruiter did not initiate (from Paraform's broader matching layer) — treated identically to a recruiter-initiated submission: it only becomes an active Command Center track after explicit recruiter confirmation
