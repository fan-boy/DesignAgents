...
6. ~~No notification channel for the Action Queue is defined.~~ **Superseded 2026-07-11** — the standalone Action Queue is gone; approval-required items now surface inline on their opportunity row, so this specific gap no longer applies. It resurfaces in a smaller form, though: a recruiter's cross-candidate backlog (everything awaiting them across every candidate they own) still needs some notification mechanism, since there's no longer even a per-candidate queue to check. Tracked as an open question rather than closed.
...
- The candidate page now has a clear hierarchy: Opportunities (default landing, task-oriented) and Profile & Context (secondary, on-demand, data-oriented). Design decisions should default to keeping recruiters on Opportunities unless they deliberately choose to go deeper.
