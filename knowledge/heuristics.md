...
Use when logging heuristic violations in design reviews or Jira.

| Severity | Label | Description | Action |
|---|---|---|---|
| S0 | Blocker | Prevents task completion or causes data loss | Must fix before launch |
| S1 | Critical | Causes significant confusion, errors, or trust failure | Must fix before launch |
| S2 | Major | Degrades experience meaningfully; workaround exists | Fix in current cycle |
| S3 | Minor | Small inconsistency or missed polish | Fix if time allows |
| S4 | Suggestion | Enhancement with no current violation | Backlog for consideration |

---

## Review Checklist

Use before every design handoff or critique session.

### Flow & Task Completion

- [ ] Can a first-time user complete the primary task without help?
- [ ] Are all error states handled with clear recovery paths?
- [ ] Do all wizard/multi-step flows support Back without data loss?
- [ ] Are destructive actions confirmed before execution?
- [ ] Are async states (loading, queued, failed) represented?

### Clarity & Language

- [ ] Is all copy in plain English (no internal jargon in learner-facing UI)?
- [ ] Are error messages specific — what happened + why + what to do?
- [ ] Are button labels action verbs?
- [ ] Is terminology consistent with the approved glossary?

### Design System Compliance

- [ ] Are all colors applied via semantic tokens (not hardcoded values)?
- [ ] Are all text elements using defined text styles?
- [ ] Are spacing values on the 4px grid?
- [ ] Do all interactive components have hover, focus, active, and disabled states?

### RBAC & Permissions

- [ ] Are permission-gated actions visually disabled with explanatory tooltip?
- [ ] Are RBAC questions flagged as open issues before handoff?
- [ ] Is no PII surfacing outside of permissioned flows?
...
- [ ] Is color never the sole carrier of meaning?
- [ ] Are all form fields associated with their labels?

### Mobile & Density

- [ ] Are touch targets >= 44x44px?
- [ ] Does the design degrade gracefully at tablet width?
